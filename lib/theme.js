...
//
//   dd: {accent, ink, inkMid, inkSoft, line, surface} — the project's
//   deep-dive page, every key optional. ddVars() below maps them to the
//   --dd-* vars the .dd-page tone channels prefer (see
//   pages/deep-dive/deep-dive.css, the one block that also assigns which
//   channel each dd element wears: hero h1, eyebrow, labels, headings, note,
//   copy). Omitted keys keep the cream light-on-dark palette; dd.accent
//   overrides the band accent for the page's --pink channel.
//
...
export const SECTIONTHEME = {
    ...
    'DADA-85': {   // sky flood, ember links, fire-red numbers, violet hover
        ...
        band: { accent: '#99e9f2', pink: '#8a3609', inkSoft: '#7f330f' }
    },
...
// The dd group → the --dd-* vars the deep-dive page's tone channels prefer.
// Applied on the .dd-page body by DeepDivePage; the print rules in
// deep-dive.css re-declare the tones directly, so print stays ink-on-white
// whatever the theme says.
export function ddVars(theme) {
    var vars = {};
    if (!theme) return vars;
    var dd = theme.dd || {};
    if (dd.accent) vars['--dd-accent'] = dd.accent;
    if (dd.ink) vars['--dd-ink'] = dd.ink;
    if (dd.inkMid) vars['--dd-ink-mid'] = dd.inkMid;
    if (dd.inkSoft) vars['--dd-ink-soft'] = dd.inkSoft;
    if (dd.line) vars['--dd-line'] = dd.line;
    if (dd.surface) vars['--dd-surface'] = dd.surface;
    return vars;
}
